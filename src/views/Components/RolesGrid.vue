<template>
  <DataGrid :data="rolesTable" title="Administración de roles" :loading="store.loadingData">
    ...
    <template #actions="{ row }">
      <button v-if="props.userID" class="btn btn-primary btn-sm me-2" @click="assignRemoveUser(row as RoleRow)" :disabled="store.loadingRole">
      <!-- <button v-if="props.userID" class="btn btn-primary btn-sm me-2"> -->
        <BootstrapIcon :icon="(row.has_role_user == '' ? 'plus' : 'dash') + '-circle-fill'" size="20" />
      </button>
      <CustomButton
        class="me-2"
        icon="gear-fill"
  ...
  <ModalBase ref="formModal" size="lg" @closed="onCloseForm">
    ...
    <ModalBase ref="permissionsModal" size="xl">
      ...
      <PermisosGrid ref="permisosGridRef" :roleID="configSelected?.id?.toString()" :loadOnMounted="false" />
    </ModalBase>
</template>
...
import PermisosGrid from './PermisosGrid.vue'
...
const loadData = async (userID: string | null = null) => {
...
const assignRemoveUser = async (row: RoleRow) => {
  if(props.userID && row.id){
    const res = !row.has_role_user ?
      await store.addRole(row.id?.toString(), props.userID) :
      await store.removeRole(row.id?.toString(), props.userID)
    ...
    // if(!row.has_role_user){
    //   store.addRole(row.id?.toString(), props.userID)
    // } else {
    //   store.removeRole(row.id?.toString(), props.userID)
    // }
  }
}
